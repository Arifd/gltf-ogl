	...
	*/
	static getMesh( name, json, bin ){
		...
		for( i=0;  i < json.nodes.length; i++ ){
			nn = json.nodes[ i ];
			if( nn.name === name && nn.mesh != undefined ){ n = nn; break; }	
		} 
		...
		for( var i=0; i < m.primitives.length; i++ ){
			...
			if( prim.indices != undefined ) 	itm.indices	= GLTF.parseAccessor( prim.indices, json, bin );
			if( attr.NORMAL != undefined )		itm.normal	= GLTF.parseAccessor( attr.NORMAL, json, bin );
			if( attr.TEXCOORD_0 != undefined  )	itm.uv		= GLTF.parseAccessor( attr.TEXCOORD_0, json, bin );
			if( attr.WEIGHTS_0 != undefined )	itm.weights	= GLTF.parseAccessor( attr.WEIGHTS_0, json, bin ); 
			if( attr.JOINTS_0 != undefined )	itm.joints	= GLTF.parseAccessor( attr.JOINTS_0, json, bin );
